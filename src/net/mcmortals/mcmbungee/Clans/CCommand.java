                ...
                if (args.length==2) {
                    if (!isInAClan((ProxiedPlayer) sender)) {
                        try {
                            ...
                            ex.printStackTrace();}
                    } else sender.sendMessage(Utility.prefix().append("You are already in a clan! Do /c leave to leave it.").color(ChatColor.RED).create());
                } else sender.sendMessage(Utility.prefix().append("Usage: §b/c create [Clan Name]").color(ChatColor.RED).create());
            //INFO COMMAND
                    ...
                    if (isInAClan((ProxiedPlayer) sender)) {
                        try {
                            ...
                            if (res.next()) {
                                ...
                                sender.sendMessage(Utility.prefix().append("Clan name: ").color(ChatColor.AQUA).append(res.getString("Name")).color(ChatColor.GREEN).bold(true).create());
                                ...
                                try {
                                    while (re.next()) {
                                        sender.sendMessage(Utility.prefix().append("> ").color(ChatColor.GOLD).bold(true).append(re.getString("PlayerName")).color(ChatColor.GREEN).bold(false).create());
                                    }
                                } catch (Exception ex) {
                                    ex.printStackTrace();
                                }
                            ...
                            ex.printStackTrace();}
                    } else sender.sendMessage(Utility.prefix().append("You are not in a clan!").color(ChatColor.RED).create());
                } else sender.sendMessage(Utility.prefix().append("Usage: §b/c info").color(ChatColor.RED).create());
            // DISBAND COMMAND :@
                        ...
                        try {
                            ...
                            if (res.next()) {
                                ...
                                if (own.equalsIgnoreCase(sender.getName())) {
                                    ...
                                    sender.sendMessage(Utility.prefix().append("Clan ").color(ChatColor.GOLD).append(name).color(ChatColor.GOLD).bold(true).append(" got disbanded successfully!").color(ChatColor.GOLD).bold(false).create());
                                } else sender.sendMessage(Utility.prefix().append("You are not the owner of your clan!").color(ChatColor.RED).create());
                ...
                } else sender.sendMessage(Utility.prefix().append("Usage: §b/c disband").color(ChatColor.RED).create());
            //LEAVE COMMAND :@
                    ...
                    if (isInAClan((ProxiedPlayer) sender)) {
                        try {
                            ...
                            if (res.next()) {
                                ...
                                } else sender.sendMessage(Utility.prefix().append("You cannot leave your own clan! Disband it instead with /c disband or transfer it by using /c transfer [Player].").color(ChatColor.RED).create());
                            } else sender.sendMessage(Utility.prefix().append("Some error occurred! Report this to staff immediately!").color(ChatColor.RED).create());
                        } catch (Exception ex) {ex.printStackTrace();}
                    } else sender.sendMessage(Utility.prefix().append("You are not in a clan!").color(ChatColor.RED).create());
                } else sender.sendMessage(Utility.prefix().append("Usage: §b/c leave").color(ChatColor.RED).create());
            //TRANSFER COMMAND
                        ...
                        try {
                            ...
                            if (res.next()) {
                                ...
                                if (own.equalsIgnoreCase(sender.getName())) {
                                    ...
                                    if (re.next()){
                                        if (re.getInt("ClanID")!=id ) { sender.sendMessage(Utility.prefix().append("You can only transfer the clan to a member of your clan!").color(ChatColor.RED).create()); return;}
                                    } else {sender.sendMessage(Utility.prefix().append("You can only transfer the clan to a member of your clan!").color(ChatColor.RED).create()); return;}
                ...
                } else sender.sendMessage(Utility.prefix().append("Usage: §b/c transfer [Player]").color(ChatColor.RED).create());
            //ΚΙΨΚ COMMAND
                        ...
                        try {
                            ...
                            if (res.next()) {
                                ...
                                if (own.equalsIgnoreCase(sender.getName())) {
                                    ResultSet re = s.executeQuery("SELECT * FROM McMPData WHERE PlayerName='" + args[1] + "'");
                                    if (re.next()){
                                        if (re.getInt("ClanID")!=id ) { sender.sendMessage(Utility.prefix().append("That player is not in your clan!").color(ChatColor.RED).create()); return;}
                                    } else {sender.sendMessage(Utility.prefix().append("That player is not in your clan!").color(ChatColor.RED).create()); return;}
                                    if (args[1].equalsIgnoreCase(sender.getName())) {
                                        sender.sendMessage(Utility.prefix().append("Cannot kick yourself from your clan!").color(ChatColor.RED).create()); return;
                                    }
                ...
                } else sender.sendMessage(Utility.prefix().append("Usage: §b/c kick [Player]").color(ChatColor.RED).create());
            //JOIN COMMAND
                            ...
                            if (res.next()) {
                                if (res.getInt("ClanID")==-1) {
                                    ...
                                    if (re.next()){
                                        ...
                                        sender.sendMessage(Utility.prefix().append("Joined clan ").color(ChatColor.GOLD).append(m).color(ChatColor.GOLD).bold(true).append(" successfully!").color(ChatColor.GOLD).bold(false).create());
                                    } else sender.sendMessage(Utility.prefix().append("Invalid join code!").color(ChatColor.RED).create());
                                } else sender.sendMessage(Utility.prefix().append("You are already in a clan!").color(ChatColor.RED).create());
                            } else sender.sendMessage(Utility.prefix().append("Some error occurred! Report this to staff immediately!").color(ChatColor.RED).create());
                        } catch (Exception ex) {ex.printStackTrace();}
                    } else sender.sendMessage(Utility.prefix().append("You are already in a clan!").color(ChatColor.RED).create());
                } else sender.sendMessage(Utility.prefix().append("Usage: §b/c join [JoinCode]").color(ChatColor.RED).create());
            } else sender.sendMessage(Utility.prefix().append("Unknown command, do §b/c §cfor help.").color(ChatColor.RED).create());
        // END OF COMMANDS
        } else sender.sendMessage(Utility.prefix().append("You cannot create a clan console!").color(ChatColor.RED).create());
    }
