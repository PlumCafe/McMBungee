    ...
    public void execute(CommandSender sender, String[] args) {
        try {
            if (m.hasPermission(sender,6)) {
                if (args.length==1) {
                    ...
                    if (res.next()) {
                        if (res.getInt("Muted")!=1) {
                            sender.sendMessage(Utility.prefix().append("This player is not muted!").color(ChatColor.RED).create()); return;
                        }
                ...
                } else sender.sendMessage(Utility.prefix().append("Usage: /unmute [Player]").color(ChatColor.RED).create());
